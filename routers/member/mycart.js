...
router.get('/:id', async (req, res, next) => {
    ...
    try {
        ...
        let [response_product] = await pool.execute(
            `SELECT user_cart.*, product.product_id,product.name,product.price,product.ins_main_id, brand.name AS brand_name,product_img.image FROM (user_cart INNER JOIN product on product.product_id = user_cart.product_id) INNER JOIN product_img on user_cart.product_id = product_img.product_id  INNER JOIN brand on brand.id = product.ins_brand WHERE user_id= ?`,
            [user_id]
        );
        //class
